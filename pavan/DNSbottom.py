#!/usr/bin/python
# Filename: DNSbottom.py

import os,time
home = os.getenv("HOME")
path = home + "/CACHEHITRATE/" 
f=open(path+'/bottom100.txt')
f1=open(path+'/bottom100.txt')
TTL=open(path+'/bottom_TTL.txt')
dname={}
ope=0
goog=0
isp=0
...
if not os.path.exists(path):
        os.mkdir(path)
f3=open(path+'/OpenResultBottom.txt','a')
f4=open(path+'/GoogleResultBottom.txt','a')
f5=open(path+'/ISPResultBottom.txt','a')
f3.write(date+time+',')
f4.write(date+time+',')
f5.write(date+time+',')
k=0
